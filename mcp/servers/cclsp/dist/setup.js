...
var esm_default8 = createPrompt((config, done) => {
  ...
  useKeypress((key2, rl) => {
    ...
    } else if (key2.name === "up" || key2.name === "down") {
      rl.clearLine(0);
      const [selectedChoice, active] = getSelectedChoice(value, choices);
      if (!selectedChoice) {
        const firstChoice = key2.name === "down" ? choices.find(isSelectableChoice) : choices.findLast(isSelectableChoice);
        setValue(firstChoice.key);
      } else if (loop || key2.name === "up" && active !== bounds.first || key2.name === "down" && active !== bounds.last) {
...
async function scanDirectoryForExtensions(dirPath, maxDepth = 3, ignoreFilter, debug = false) {
  ...
  async function scanDirectory(currentPath, currentDepth, relativePath = "") {
    ...
    try {
      ...
      if (debug) {
        console.log(`Scanning directory ${currentPath} (depth: ${currentDepth}), found ${entries.length} entries: ${entries.join(", ")}`);
      }
...
\uD83D\uDD04 Adding cclsp to Claude MCP configuration...`);
      ...
      try {
        const mcpArgs = mcpCommand.split(" ").slice(1);
...
var isMainModule = import.meta.url.startsWith("file://") && (process.argv[1].endsWith("setup.js") || process.argv[1].endsWith("setup.ts"));
if (isMainModule) {
  main().catch((error) => {
    console.error(`
